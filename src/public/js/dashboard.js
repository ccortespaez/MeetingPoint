...
const showTab = n => {
    ...
    if (n == (x.length - 1)) {
        let button = document.getElementById("nextBtn").classList.add('d-none');
        let buttons = document.querySelector('.buttons');
        let buttonSubmit = document.createElement('button');
        buttonSubmit.classList.add('btn', 'bg-primary');
        buttonSubmit.innerHTML = "Crear evento"
        buttonSubmit.setAttribute('href', 'submit');
        buttons.appendChild(buttonSubmit);
    } else {    
        let button = document.getElementById("nextBtn").classList.remove('d-none').innerHTML = "Continuar";
    }   
    fixStepIndicator(n);
}
...
const nextPrev = n => {
    ...
    if (currentTab >= x.length) {
        document.getElementById("nextprevious").style.dsisplay = "none";
